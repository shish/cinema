...
describe('minititle', () => {
    it('returns title unchanged when folder is empty', () => {
        expect(minititle('', 'My Series - S01E01 - Pilot.mkv')).toBe('My Series - S01E01 - Pilot.mkv');
    });

    it('removes folder prefix and leading separators', () => {
        expect(minititle('My Series', 'My Series - S01E01 - Pilot.mkv')).toBe('S01E01 - Pilot.mkv');
    });
    ...
    it('removes leading separators even when folder does not match', () => {
        expect(minititle('Series', ' - S01E01 - Pilot.mkv')).toBe('S01E01 - Pilot.mkv');
    });
