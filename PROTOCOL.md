...
* User connects to `/api/room?room=...&user=...&sess=...` to get a websocket connection for sending commands
  * `room` -- room code, a new room will be created if one doesn't exist
  * `user` -- username, any arbitrary text
  * `sess` -- client-chosen GUID
* User subscribes to MQTT topic `rooms/{room_code}/state` to receive room state updates
* User can send commands via the websocket:
